...
const healthSignals = agents.map((agent) => {
  ...
  const statusRow = statusRows.find((row) => row.name === agent.name);
  const cron = cronRows.find((row) => row.agent === agent.name);
  const failingJobs = (cron?.jobs || []).filter((job) => job.health === "failing").length;
  ...
  return {
    name: agent.name,
    status: failingJobs ? "Watch" : connection.status,
    load: statusRow?.health ? `${statusRow.health}${cron?.source ? ` · ${cron.source}` : ""}` : connection.load,
    blocker: statusRow?.blocker || connection.blocker,
    cron: cron?.source || "No live cron feed connected",
    score: Math.max(10, connection.score - failingJobs * 8),
    trend: failingJobs ? "Declining" : connection.trend,
    lastAction: lastAudit?.title || connection.lastAction,
    lastCorrection: correction?.title || "No live correction logged",
    checkIn: statusRow?.lastSeen ? formatTime(statusRow.lastSeen) : connection.checkIn,
  };
});

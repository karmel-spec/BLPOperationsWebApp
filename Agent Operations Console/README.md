# BLP Agent Operations Console

One-stop console for the BLP human team to message, train, correct, and request
cron jobs from the agent workforce.

## Live vs grayed agents (2026-07-07)

`data/registry-connections-data.js` is transcribed from the **BLP OpenClaw Agent
Connection Details** registry sheet
(https://docs.google.com/spreadsheets/d/1uB5vYpNwld1HbuGuhG_qNRdjS2Czg0mwOeut1eSIlAI/edit).

- **LIVE (6)** — Lindsay, Cody, Ivory, Melody, Chris, Libby. Full verified
  connection info. Console actions are enabled: Message / Assign / Correct /
  Train / Request Cron Job. Each action is recorded in the Agent Log (and the
  local API queue when running), then opens a prefilled Gmail draft to the
  agent's registry intake address — the human stays the sender of record,
  which keeps every send inside the registry's approval boundaries.
- **Setup needed (2)** — Walter (REST wrapper agreed but not built; no payload,
  test task, or cron enumeration), Dawn (mailbox bounces 550; OAuth not wired).
  Grayed out with a "Missing for go-live" checklist.
- **Awaiting registry (3)** — Marcus, Monte, Sally. Active in the roster but
  their registry columns are empty. Grayed out with a checklist.
- **On deck (30)** — everyone else. Grayed out until they're built.

To flip an agent live: fill in its column in the registry sheet, add/update its
record in `data/registry-connections-data.js` with `readiness: "live"`, update
`data/agent-status.json` + `data/cron-inventory.json`, and rerun
`node scripts/build-live-operational-data.mjs`.

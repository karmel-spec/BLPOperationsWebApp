...
const registryConnections = window.BLP_REGISTRY_CONNECTIONS?.agents || {};
const registryUrl = window.BLP_REGISTRY_CONNECTIONS?.registryUrl || "#";

function connectionFor(agent) {
  return registryConnections[agent.name] || null;
}

function isLiveAgent(agent) {
  return connectionFor(agent)?.readiness === "live";
}

function connectionStateLabel(agent) {
  const connection = connectionFor(agent);
  if (!connection) return "On Deck — not built yet";
  if (connection.readiness === "live") return "LIVE — connected";
  if (connection.readiness === "partial") return "Setup needed";
  return "Awaiting registry details";
}

function emailChannel(agent) {
  return (connectionFor(agent)?.liveChannels || []).find((channel) => channel.kind === "email") || null;
}
...
function missingChecklist(agent) {
  const connection = connectionFor(agent);
  const missing = connection?.missing || [];
  if (connection && connection.readiness === "live") return "";
  const items = missing.length
    ? missing
    : ["Add this agent to the connection-details registry (intake, status source, cron list, permissions, safe test)."];
  return `
    <div class="connection-checklist">
      <span>Missing for go-live</span>
      <ul>${items.map((item) => `<li>${item}</li>`).join("")}</ul>
      <a href="${registryUrl}" target="_blank" rel="noreferrer">Open agent registry sheet</a>
    </div>
  `;
}
...
function enrichAgent(agent) {
  ...
  return {
    ...agent,
    connection,
    isLive: connection?.readiness === "live",
    ...
    permissions: connection?.permissions?.drafts?.length ? connection.permissions.drafts : overrides.permissions || defaults.permissions,
    guardrails: connection?.permissions?.approvals?.length ? connection.permissions.approvals : overrides.guardrails || defaults.guardrails,
    automations: connection
      ? connection.cronJobs.map((job) => job.name)
...
function renderDrawer(agent = selectedAgent) {
  ...
  const live = agent.isLive;
  const primaryChannel = connection?.liveChannels?.[0];
  const cronSummary = connection
    ? connection.cronJobs.length
      ? `${connection.cronJobs.length} recurring job${connection.cronJobs.length === 1 ? "" : "s"}${connection.cronJobs.some((job) => job.health === "failing") ? " · ⚠ failures" : ""}`
      : "No recurring jobs yet"
    : `${agent.automations[0]} · not connected`;
  const actionButtons = live
    ? `
      ...
      >Train</button>
      <button class="secondary-light-button" type="button" data-agent-action="Request Cron">Request Cron Job</button>
    `
    : `<p class="drawer-disabled-note">Actions unlock when this agent's registry row is complete and its connection is verified.</p>`;
  agentDrawer.innerHTML = `
    <header class="drawer-header">
      ...
      <span class="connection-pill ${live ? "live" : "offline"}">${connectionStateLabel(agent)}</span>
    </header>
    ...
    <div class="drawer-action-grid ${live ? "" : "is-disabled"}">
      ${actionButtons}
    </div>
    ${missingChecklist(agent)}
    <div class="detail-list">
      ${live && primaryChannel ? `<div><span>Live channel</span><strong>${primaryChannel.label}${primaryChannel.address ? ` · ${primaryChannel.address}` : ""}</strong></div>` : ""}
      ${connection?.statusSource ? `<div><span>Status source</span><strong>${connection.statusSource}</strong></div>` : `<div><span>Current work</span><strong>${getAgentWork(agent)[0]}</strong></div>`}
      <div><span>Supervisor</span><strong>${agent.supervisor}</strong></div>
      <div><span>Email</span><strong>${agent.email}</strong></div>
      <div><span>Cron</span><strong>${cronSummary}</strong></div>
...
const agentActionCopy = {
  ...
  "Request Cron": {
    eyebrow: "Request cron job",
    prompt: "What should run automatically, and on what schedule?",
    helper: "Describe the recurring job: schedule, inputs, outputs, and who receives the result. Cron changes always require the approver listed in the registry (usually Karmel) before the agent enables them.",
    placeholder: "Example: Every weekday at 7:30 AM, summarize new tuning requests and email the summary to walter@brighamlarsonpianos.com.",
  },
};
...
function saveCurrentAgentAction(event) {
  ...
  const risk = action === "Request Cron" ? "High" : action === "Train" || action === "Correct" ? "Medium" : "Low";
  const logRecord = {
    time: "Just now",
    agent: agent.name,
    human: "BLP team",
    ...
    receipt: action === "Train"
      ...
      : action === "Correct"
        ...
        : action === "Assign Task"
          ...
          : action === "Request Cron"
            ? "Cron request recorded — routed to the agent and its approver."
  ...
  openLiveDispatch(action, agent, notes);
}

// For live agents whose registry intake is email, open a prefilled Gmail compose
// window so the human sends from their own account — the human stays the sender
// of record, matching the registry's approval boundaries.
function openLiveDispatch(action, agent, notes) {
  if (!agent.isLive) return;
  const channel = emailChannel(agent);
  if (!channel?.address) return;
  const connection = agent.connection;
  const subjectPrefix = action === "Request Cron" ? "Cron request" : action;
  const subject = `[BLP Console] ${subjectPrefix} for ${agent.name}`;
  const bodyLines = [
    `${agent.name},`,
    "",
    notes,
    "",
    action === "Request Cron"
      ? `Approver required before enabling: ${connection?.cronApprover || "Karmel"}. Please draft the job spec (name, schedule, inputs, outputs, failure behavior) and send it for approval — do not enable it yet.`
      : action === "Train"
        ? "Treat this as reusable guidance. Route any Knowledge Vault change through Libby for Karmel's approval."
        : action === "Correct"
          ? "Please revise this specific output and reply with the corrected version for review."
          : "Reply with a confirmation or your draft. Anything customer-facing stays draft-only until approved.",
    "",
    "— Sent from the BLP Agent Operations Console",
  ];
  window.open(gmailComposeUrl(channel.address, subject, bodyLines.join("\n")), "_blank", "noopener");
}
...
function renderDashboard(agent = selectedAgent) {
  ...
  agentDashboard.innerHTML = `
    ...
    <div class="dashboard-hero">
      ...
      <div class="dashboard-hero-actions">
        ...
        <button class="secondary-light-button" type="button" data-dashboard-action="Request Cron">Request Cron Job</button>`
          : `<span class="connection-pill offline">${connectionStateLabel(agent)}</span>`}
      </div>
    </div>
    ...
    <div class="dashboard-grid">
      <section class="dash-panel span-2">
        ...
        <div class="brief-grid">
          ...
          <div><span>Supervisor</span><strong>${agent.supervisor}</strong></div>
          <div><span>Email</span><strong>${agent.email}</strong></div>
          <div><span>Channels</span><strong>${agent.connection?.liveChannels?.length ? agent.connection.liveChannels.map((channel) => channel.kind).join(" · ") : "Not connected yet"}</strong></div>
        </div>
      </section>

      <section class="dash-panel span-2 connection-panel ${agent.isLive ? "is-live" : "is-offline"}">
        <div class="section-heading compact"><h3>Connection</h3><span class="connection-pill ${agent.isLive ? "live" : "offline"}">${connectionStateLabel(agent)}</span></div>
        ${agent.connection
          ? `
          <div class="brief-grid">
            <div><span>Runtime</span><strong>${agent.connection.runtime?.system || agent.system}${agent.connection.runtime?.model ? ` · ${agent.connection.runtime.model}` : ""}${agent.connection.runtime?.host ? ` · ${agent.connection.runtime.host}` : ""}</strong></div>
            ${agent.connection.statusSource ? `<div><span>Status source</span><strong>${agent.connection.statusSource}</strong></div>` : ""}
            ${agent.connection.safeTest ? `<div><span>Safe connection test</span><strong>${agent.connection.safeTest.expected}</strong></div>` : ""}
            ${agent.connection.redaction ? `<div><span>Redaction</span><strong>${agent.connection.redaction}</strong></div>` : ""}
          </div>
          ${agent.connection.payloadHint ? `<p class="payload-hint"><span>Task payload</span><code>${agent.connection.payloadHint.replaceAll("<", "&lt;")}</code></p>` : ""}
          ${agent.connection.notes ? `<p class="connection-notes">${agent.connection.notes}</p>` : ""}
          `
          : `<p class="connection-notes">This agent is on deck. It has no runtime, registry row, or channels yet — it appears here so the team can see the planned org.</p>`}
        ${missingChecklist(agent)}
      </section>
      ...
      <section class="dash-panel">
        <div class="section-heading compact"><h3>Cron Jobs</h3>${agent.isLive ? `<button class="text-button" type="button" data-dashboard-action="Request Cron">Request job</button>` : `<span class="pill">Not connected</span>`}</div>
        ${agent.connection
          ? agent.connection.cronJobs.length
            ? agent.connection.cronJobs.map((job) => `<article class="cron-item ${job.health === "failing" ? "is-failing" : ""}"><strong>${job.name}</strong><span>${job.schedule}</span><small>${job.health === "failing" ? `⚠ FAILING — ${job.note || "needs attention"}` : "Last run OK"}${job.approver ? ` · Approver: ${job.approver}` : ""}</small></article>`).join("")
            : `<article class="cron-item"><strong>No recurring jobs yet</strong><span>—</span><small>${agent.isLive ? "Use Request Cron Job to propose one." : "Available once connected."}</small></article>`
          : agent.automations.map((job) => `<article class="cron-item is-planned"><strong>${job}</strong><span>Planned</span><small>Activates when this agent is connected</small></article>`).join("")}
      </section>
      ...
      <section class="dash-panel span-2">
        <div class="section-heading compact"><h3>Communication Center</h3>${agent.isLive && emailChannel(agent) ? `<span class="pill">Live · sends via your Gmail</span>` : `<span class="pill">Offline</span>`}</div>
        ${agent.isLive
          ...
          <div class="request-controls">
            <select aria-label="Message purpose"><option>Message</option><option>Assign Task</option><option>Correct</option><option>Train</option><option>Request Cron</option></select>
            <button class="primary-button" type="button" id="composerSend">Review &amp; Send</button>
          </div>
          <small class="composer-note">Recorded in the Agent Log and Work Queue, then opened as a prefilled Gmail draft to ${emailChannel(agent)?.address || agent.email} — you stay the sender of record.</small>
        </div>`
          : `<p class="connection-notes">Messaging unlocks when ${agent.name} is connected. ${agent.connection ? "See the Connection panel for what's missing." : "This agent hasn't been built yet."}</p>`}
      </section>
    </div>
  `;
  ...
  const composerSend = agentDashboard.querySelector("#composerSend");
  if (composerSend) {
    composerSend.addEventListener("click", () => {
      const composer = agentDashboard.querySelector(".message-composer textarea");
      const purpose = agentDashboard.querySelector(".message-composer select");
      openAgentAction(purpose?.value || "Message", agent);
      if (composer?.value.trim()) agentActionNotes.value = composer.value.trim();
    });
  }
...
function renderAgents() {
  ...
  agentGrid.innerHTML = filtered
    .map(
      (agent) => `
      <article class="agent-card ${isLiveAgent(agent) ? "is-live" : "is-not-connected"}">
        <button type="button" data-agent="${agent.name}">
          ...
          <span class="connection-pill ${isLiveAgent(agent) ? "live" : "offline"}">${connectionStateLabel(agent)}</span>

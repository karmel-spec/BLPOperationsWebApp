...
The module at `modules/shop-reports/` replaces the weekly technician report
...
- **Dashboard** — who has / hasn't reported this week, per-tech submission
  ...
  (`modules/shop-reports/data/report-history.json`, 4,538 entries) as fallback.
...
2. Paste the contents of `modules/shop-reports/shop-reports-apps-script.gs`
...
5. In `modules/shop-reports/index.html`, set `CONFIG.APPS_SCRIPT_URL` to that
...
2. Set `CONFIG.GOOGLE_CLIENT_ID` in `modules/shop-reports/index.html`.
...
`modules/shop-reports/data/report-history.json`.
